...
public class RevealAnimation {
    ...
    public RevealAnimation(View view, Bundle bundle, Activity activity) {
        mView = view;
        mActivity = activity;

        //when you're android version is at leat Lollipop it starts the reveal activity
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP &&
                bundle.containsKey(EXTRA_CIRCULAR_REVEAL_X) &&
                bundle.containsKey(EXTRA_CIRCULAR_REVEAL_Y)) {
            view.setVisibility(View.INVISIBLE);

            revealX = bundle.getInt(EXTRA_CIRCULAR_REVEAL_X, 0);
            revealY = bundle.getInt(EXTRA_CIRCULAR_REVEAL_Y, 0);
